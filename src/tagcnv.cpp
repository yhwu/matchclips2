...
int check_readdepth(int argc, char* argv[])
{
  ...
  while ( !FIN.eof() ) {
	...
	//  >> mapq 
      ...
      POS1=atoi(POS.c_str());
      POSCIGAR_st m;
      resolve_cigar_pos(POS1, CIGAR, m);  
      if ( m.pos<=0 ) continue;
      vector<int> rpos(SEQ.size(),0);
      expand_pos(m, rpos);
      if ( rpos.size()!=SEQ.size() ) {
	cerr << "Error understanding CIGAR and POS " 
	     << POS << "\t" << CIGAR << "\t" << SEQ.size() << endl
	     << POS1 << "\t" << get_cigar(m) << "\t" << rpos.size() << endl;
      }
      
      for(size_t k=0;k<rpos.size();++k) {
	if ( rpos[k]>p1-(int)SEQ.size() && rpos[k]<p1 ) ++dout;
	if ( rpos[k]>p2 && rpos[k]<p2+(int)SEQ.size() ) ++dout;
	if ( rpos[k]==p1 ) ++dp1;
	if ( rpos[k]==p2 ) ++dp2;
  ...
  */
  ...
  while ( !FIN.eof() ) {
    ...
    */
    ...
    bool NOSEQ=false;
    if ( POS1>=(int)FASTA.length() || POS2>=(int)FASTA.length() ) NOSEQ=true;
    else {
...
int variation_to_fastq(int argc, char* argv[])
{
  ...
  */
  ...
  while ( !FIN.eof() ) {
    ...
    POSCIGAR_st m;
    resolve_cigar_pos(POS, CIGAR, m);  
    string readType= (m.anchor<=m.iclip) ? "MS":"SM";
    int nM=0,nS=0,nID=0;
    for(int i=0;i<(int)m.op.size();++i) {
      if ( m.op[i]==BAM_CMATCH ) nM+=m.nop[i];
      if ( m.op[i]==BAM_CSOFT_CLIP ) nS+=m.nop[i];
      if ( m.op[i]==BAM_CINS ) nID+=0;
      if ( m.op[i]==BAM_CDEL || m.op[i]==BAM_CREF_SKIP ) nID+=m.nop[i];
    }
